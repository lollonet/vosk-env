...
class VoiceInject:
    def __init__(self, server_url="ws://localhost:8765"):
        ...
        self.dev_corrections = {
            ...
            "claude aiuto": "help me",
            "claude crea": "create",
            "claude spiega": "explain",
        }
    ...
    def inject_text_to_terminal(self, text):
        ...
        try:
            ...
            subprocess.run(
                ["xdotool", "type", text], check=False, stderr=subprocess.DEVNULL
            )
            return True
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Claude Voice Inject - Voice input per sessioni Claude attive"
    )
    parser.add_argument(
        "--server", default="ws://localhost:8765", help="URL voice server"
    )
    parser.add_argument(
        "--quick", "-q", action="store_true", help="Single voice capture"
    )
