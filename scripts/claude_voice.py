...
class ClaudeVoice:
    def __init__(self, server_url='ws://localhost:8765'):
        ...
        self.dev_corrections = {
            # Linguaggi
            'paiton': 'python', 'python': 'python',
            'giava script': 'javascript', 'javascript': 'javascript',
            'java script': 'javascript', 'node jes': 'nodejs',
            'react': 'react', 'riact': 'react',
            'view jes': 'vue.js', 'angular': 'angular',
            ...
            # Database
            'mai sequel': 'mysql', 'postgres': 'postgresql',
            'mongo db': 'mongodb', 'redis': 'redis',
            ...
            'microservizi': 'microservices', 'ci cd': 'CI/CD',
            'devops': 'DevOps', 'cloud native': 'cloud native',
    ...
    async def interactive_session(self):
        ...
        print(f"📁 Contesto rilevato: {context.get('main_language', 'Unknown')} in {os.path.basename(context['pwd'])}")
        print()
        ...
        try:
            while True:
                ...
                if choice == 'e':
                    edited = input(f"✏️ Modifica prompt:\n> ")
                    if edited.strip():
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='Claude Voice - Voice input per Claude Code')
    parser.add_argument('--interactive', '-i', action='store_true',
                       help='Modalità interattiva')
    parser.add_argument('--quick', '-q', action='store_true',
                       help='Prompt singolo veloce')
    parser.add_argument('--prompt', '-p', type=str,
                       help='Prompt diretto (senza voice)')
    parser.add_argument('--server', default='ws://localhost:8765',
                       help='URL voice server')
