...
LINUX_COMMANDS = {
    ...
    'liste la': 'ls -la', 'ci di': 'cd', 'vai in': 'cd',
    'pi uadiblu': 'pwd', 'dove sono': 'pwd',
    'tocca': 'touch', 'crea file': 'touch', 'mkdir': 'mkdir',
    'copia': 'cp', 'sposta': 'mv', 'rimuovi': 'rm',
    'cat': 'cat', 'mostra': 'cat', 'grep': 'grep',
    'pi es': 'ps', 'processi': 'ps aux', 'top': 'top',
    'df': 'df -h', 'spazio disco': 'df -h',
    'free': 'free -h', 'memoria': 'free -h',
    'ping': 'ping', 'wget': 'wget', 'curl': 'curl',
    'git': 'git', 'git status': 'git status', 'stato git': 'git status',
    'git add': 'git add', 'git commit': 'git commit',
    'git push': 'git push', 'pus': 'git push',
    'docker': 'docker', 'container': 'docker ps',
    'sudo': 'sudo', 'installa': 'sudo apt install',
    ...
    try:
        async for message in websocket:
            ...
            if msg_type == 'start_permanent_listening':
                ...
                current_engine = engines.get(current_language)
                if current_engine:
                    def single_callback(text, confidence):
                        corrected_text = correct_text(text, 'terminal')
                        original = text if corrected_text != text else None
                    ...
                    threading.Thread(
                        target=lambda: current_engine.start_listening(
...
async def main():
    ...
    print(f"🖥️  Supporto Browser: Permanente + switch lingua")
    print(f"💻 Supporto Terminal: Single capture + comandi Linux")
