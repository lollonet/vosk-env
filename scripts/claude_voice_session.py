    ...
    async def start_claude_session(self, claude_args=None):
        ...
        try:
            ...
            await asyncio.gather(
                voice_task, keyboard_task, output_task,
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='Claude Code con Voice Input Integrato')
    parser.add_argument('--computer-use', action='store_true',
                       help='Avvia in modalità computer use')
    parser.add_argument('--chat', action='store_true',
                       help='Avvia in modalità chat (default)')
    parser.add_argument('--server', default='ws://localhost:8765',
                       help='URL voice server')
    parser.add_argument('claude_args', nargs='*',
                       help='Argomenti aggiuntivi per Claude Code')
