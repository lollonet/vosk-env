        ...
        if data.get('type') == 'speech_result':
            ...
            if text:
                ...
                if self.is_dangerous_command(corrected_command):
                    confirm = input("⚠️  Comando potenzialmente pericoloso. Eseguire? (y/N): ")
                    if confirm.lower() != 'y':
                        print("❌ Comando annullato")
                        return
                
                # Chiedi se eseguire
                choice = input("💾 [Enter]=Esegui [e]=Edita [s]=Salta: ").strip().lower()
...
async def main():
    ...
    parser.add_argument('--server', default='ws://localhost:8765',
                       help='URL server WebSocket')
    parser.add_argument('--test', action='store_true',
                       help='Test correzioni comandi')
