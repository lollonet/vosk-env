...
def migrate_legacy_config(old_command_map: Dict[str, str], output_path: Path) -> None:
    ...
    for voice_cmd, shell_cmd in old_command_map.items():
        ...
        safe = not any(dangerous in shell_cmd for dangerous in [";", "&", "|", "`", "$"])
    ...
    if any(not cmd.get("safe") for cmd in new_config["voice_commands"].values()):
        print("⚠️  Some commands were marked as unsafe - review the configuration manually")
